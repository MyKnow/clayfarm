...
- 이메일 발송·이메일/MFA 코드 검증 전에 OS 저장소의 쓰기·읽기·삭제를 사전 확인한다. 임시 비밀 없는 항목만 사용하며 기존 세션·노드 키에 쓰지 않는다. 점검 후 다시 잠기는 경쟁 조건까지 제거하지는 않는다.
- 미완료 MFA 등록 키는 접속 서버·상태 폴더·사용자를 구분한 OS 자격증명 저장소에만 보관해 재실행에 재사용하며 검증 성공 시 제거한다. JSON·비대화형 출력으로 키를 표시하지 않고, 이미 검증된 인증기를 재등록 요청으로 해제하지 않는다.
